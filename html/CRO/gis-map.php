...
$userName = htmlspecialchars($_SESSION['fullname'] ?? 'CRO Officer');
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>BFP CRO — GIS Establishment Map</title>
...
<div class="main-container">
  <div class="page-header">
    ...
    <p class="mb-0">Real-time map of all active establishments with compliance status</p>
  </div>
...
async function loadMarkers() {
  try {
    ...
    if (!data.success) { alert('Failed to load GIS data: ' + data.message); return; }
...
function renderMarkers(markers) {
  ...
  markers.forEach(m => {
    ...
    const certInfo = m.certificateNumber
      ? `<br><strong>FSIC:</strong> ${m.certificateNumber} (expires ${m.certExpiry || 'N/A'})`
      : '';
    const compBadge = {
      compliant: '<span class="badge bg-success">Compliant</span>',
      partially_compliant: '<span class="badge bg-warning text-dark">Partial</span>',
      non_compliant: '<span class="badge bg-danger">Non-Compliant</span>',
    }[m.complianceStatus] || '<span class="badge bg-secondary">No Inspection</span>';
...
function applyFilter() {
  ...
  const filtered = allMarkers.filter(m => {
    const compMatch = comp === 'all' ? true
      : comp === 'no_inspection' ? !m.complianceStatus
      : m.complianceStatus === comp;
    const typeMatch = type === 'all' ? true : m.type === type;
